...
async def main(ip: str):
    async with LinWave(ip) as lw:
        ...
        await lw.set_ddt(channel=0, microseconds=10000)  # set duration discrimination time to 400 µs
        await lw.set_status_interval(channel=0, seconds=0)  # generate status data every 2 seconds
        await lw.set_threshold(channel=0, microvolts=1000)  # 1000 µV = 60 dB(AE)
        ...
        await lw.set_tr_pretrigger(channel=0, samples=0)  # 200 pre-trigger samples
        await lw.set_tr_postduration(channel=0, samples=0)  # 0 post-duration samples
